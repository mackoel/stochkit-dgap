	...
	for( unsigned int i=0; i<ReactionsList.size(); ++i){
		...
		if(cur_reaction->Type == 2){
			...
			if(cur_reaction->Reactants.size() != 0 ){
						...
						std::cerr << "StochKit WARNING (Input::linkSpeciesAndReactions): reactant " << cur_reaction->Reactants[j].Id << " not shown in the propensity function of reaction " << cur_reaction->Id << ", this may cause negative population of this species." << std::endl;
                	                }
	                        }
			}
		}
